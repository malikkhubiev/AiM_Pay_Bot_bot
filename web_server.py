...
def web_server():
    ...
    async def send_invite_link(request):
        try:
            ...
            await bot.send_message(
                chat_id=tg_id,
                text=f"Поздравляем! Всё прошло успешно 🎉. Вот ссылка для присоединения к нашей группе. Обращайтесь с ней очень аккуратно. Она одноразовая и если вы воспользуетесь единственным шансом неверно, исправить ничего не получится: {link}",
                reply_markup=keyboard
            )
            return web.json_response({"status": "notification sent"}, status=200)

...
def web_server():
    ...
    async def send_invite_link(request):
        try:
            ...
            text_info = f"Поздравляем! Всё прошло успешно 🎉. Вот ссылка для присоединения к нашей группе. Обращайтесь с ней очень аккуратно. Она одноразовая, действует 30 минут и если вы воспользуетесь единственным шансом неверно, исправить ничего не получится: {link}."
            # При официальном
            # if payment_id:

...
async def start(message: types.Message, telegram_id: str = None, username: str = None):
    ...
    if response["status"] == "success":
        if response["type"] == "temp_user":
            ...
            # Без
            info_text = response["response_message"] + "\n\n💎Мы очень рады видеть тебя, middle разработчик!💎\n\nРасширить свой стек технологиями из ML и стать специалистом более высокого уровня, ты можешь после оплаты курса\n\nДумаешь, что курс будет слишком сложным? Нажми на кнопку и получи бесплатный пробный период для ознакомления СО ВСЕМИ материалами курса.\nМы идём навстречу, чтобы принести максимум пользы middle разработчикам)"
...
async def getting_started(message: types.Message, telegram_id: str, u_name: str = None):
    ...
    if response["status"] == "success":
        ...
        await bot.send_video(
            chat_id=message.chat.id,
            video=START_VIDEO_URL,
            caption="💎Мы очень рады видеть тебя, middle разработчик!💎\n\nРасширить свой стек технологиями из ML и стать специалистом более высокого уровня, ты можешь после оплаты курса\n\nДумаешь, что курс будет слишком сложным? Нажми на кнопку и получи бесплатный пробный период для ознакомления СО ВСЕМИ материалами курса. Мы идём навстречу, чтобы принести максимум пользы middle разработчикам)",
            reply_markup=keyboard
        )
    elif response["status"] == "error":
        await message.answer(response["message"])

...
async def start(message: types.Message, telegram_id: str = None, username: str = None):
    ...
    if response["status"] == "success":
        if response["type"] == "temp_user":
            ...
            # Без
            info_text = response["response_message"] + "\n\n💎Мы очень рады тебя видеть!💎\n\nНачав обучение, ты получаешь доступ к:\n- Готовому исходному коду каждого урока\n- Нескольким сотням видеоматериалов\nВозможности построить свою первую модель уже сегодня с абсолютного нуля\n- Возможности получить стильный сертификат после прохождения\n\nЖдём тебя внутри)"
